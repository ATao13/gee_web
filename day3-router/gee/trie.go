...
type node struct {
	...
	part     string //路由中一部分，例如【doc,intro】
	children []*node
	isWild   bool //是否精准匹配，part 含有：或者*为true
}
...
func (n *node) insert(pattern string, parts []string, height int) {
	//len(parts)必须大于等于 height 否则会异常报错
	...
	if len(parts) == height {
		n.pattern = pattern //只有最底层节点才会有pattern
		return
	}
	part := parts[height]
	child := n.matchChild(part) //查询part 是否存在
	if child == nil {
