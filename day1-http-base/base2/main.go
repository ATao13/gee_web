...
//还记得吗，在实现Engine之前，我们调用 http.HandleFunc 实现了路由和Handler的映射，也就是只能针对具体的路由写处理逻辑。
//比如/hello。但是在实现Engine之后，我们拦截了所有的HTTP请求，拥有了统一的控制入口。
//在这里我们可以自由定义路由映射的规则，也可以统一添加一些处理逻辑，例如日志、异常处理等。
func main() {
	engine := new(Engine) //返回指针类型
	log.Fatal(http.ListenAndServe(":6666", engine))
}

//此处体现的inface 的可拓展性。使用者可以通过 实现接口的方式进行逻辑处理
